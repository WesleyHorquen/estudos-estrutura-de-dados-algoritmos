...
PONT criarNovoNo(TIPOCHAVE ch)
{
    ...
    novoNo->dir = NULL;
    novoNo->chave = ch;
    printf("No %d criado com sucesso!\n", ch);
    return novoNo;
}
...
bool inserirNo(PONT *raiz, TIPOCHAVE novaChave)
{
    if (*raiz == NULL)
    {
        *raiz = criarNovoNo(novaChave);
        ...
        return inserirAux(*raiz, novaChave);
...
//Exibe arvore Em Ordem - Esquerda, Raiz, Direita
...
//Exibe arvore Pre Ordem - Raiz, Esquerda, Direita
...
//Exibe arvore Pos Ordem - Esquerda, Direita, Raiz
...
int main(){
    PONT raiz;              // Ponteiro para a raiz da árvore
    inicializar(&raiz);     // Começa com a árvore vazia
    ...
    printf("\n> Arvore em ordem (esq, raiz, dir): ");
    exibirArvoreEmOrdem(raiz);
    printf("\n");

    printf("> Arvore pre-ordem (raiz, esq, dir): ");
    exibirArvorePreOrdem(raiz);
    printf("\n");

    printf("> Arvore pos-ordem (esq, dir, raiz): ");
    ...
    // Testando busca
    int chaveBusca = 40;
    PONT resultado = buscaBinaria(chaveBusca, raiz);
    if (resultado)
    ...
    // Liberando memória ao final
    destruirArvore(&raiz);
    printf("\n> Memoria liberada, arvore destruida!\n");
